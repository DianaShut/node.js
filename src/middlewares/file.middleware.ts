...
import { avatarConfig } from "../constants/file.constants";
...
class FileMiddleware {
  public isAvatarValid(req: Request, res: Response, next: NextFunction) {
    try {
      const avatar = req.files?.avatar as UploadedFile; // UploadedFile - тип для файлів, які завантажуються через fileupload
      if (!avatar) {
